...
class AbstractMemberProposal

//class AbstractMemberProposal(fullyQualifiedEnclosingType: Option[String], method: String, target: AddMethodTarget, compilationUnit: ICompilationUnit, pos: Position) extends IJavaCompletionProposal {
//
...
//    yield for ((name, tpe) <- parameterList) yield (name, tpe) //to check [Sprinter] - do we need to remove full prefix? - (name, tpe.substring(tpe.lastIndexOf('.') + 1))
//  private val parameters = parametersWithSimpleName
